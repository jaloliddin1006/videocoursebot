...
class BotUsers(BaseModel):
    ...
    telegram_phone_number = models.CharField(max_length=12, null=True, blank=True)
...
class Order(BaseModel):
    ...
    def save(self, *args, **kwargs):
        if self.user.promo_code:
            self.total_price_with_discount = self.total_price - self.user.promo_code.discount
        else:
            self.total_price_with_discount = self.total_price
        super(Order, self).save(*args, **kwargs)
    
    def promo_code(self):
        return self.user.promo_code
